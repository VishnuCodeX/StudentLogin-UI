          ...
          <table className="w-full overflow-hidden rounded-xl border border-border text-sm">
            <tbody>
              <tr className="border-b border-border">
                ...
                <td className="px-4 py-3 text-center font-bold text-success">{creds?.userName || "—"}</td>
              </tr>
              <tr>
                <td className="border-r border-border bg-muted/50 px-4 py-3 text-center font-bold text-primary">Password</td>
                <td className="px-4 py-3 text-center font-bold text-success">{creds?.password || "—"}</td>
              </tr>
            </tbody>
          </table>
...
export default function Fees() {
  ...
  if (loading) {
    return <div className="flex h-64 items-center justify-center text-muted-foreground"><Loader2 className="mr-2 h-5 w-5 animate-spin" /> Loading fee challans…</div>;
  }
  if (error) {
  ...
  return (
    <div className="space-y-6">
      ...
      {challans.length === 0 ? (
        <Card><CardContent className="py-16 text-center text-muted-foreground">No fee challans found.</CardContent></Card>
      ) : (
        <div className="space-y-2">
          {challans.map((c) => (
            <button key={c.id} onClick={() => setSelectedId(c.id)}
              className="flex w-full items-center gap-4 rounded-3xl border border-border bg-card p-4 text-left shadow-soft transition-all hover:-translate-y-0.5 hover:shadow-pop">
      ...
      <Card className="mx-auto max-w-2xl print:hidden">
        <CardContent className="p-6 sm:p-8">
          <div className="mb-6 border-b border-border pb-4 text-center">
            <h2 className="font-display text-xl font-bold">Mount Carmel College, Bengaluru</h2>
          ...
          <table className="mb-4 w-full text-sm">
            ...
            <tfoot>
              <tr className="border-t border-border"><td className="py-2 text-right font-medium">Total</td><td className="py-2 text-right tabular-nums">{rupee(c.totalAmount)}</td></tr>
              ...
              <tr className="border-t-2 border-foreground/20"><td className="py-2.5 text-right font-bold">Balance Payable</td><td className="py-2.5 text-right font-display text-lg font-bold">{rupee(c.balance)}</td></tr>
            </tfoot>
          </table>
  ...
  return (
    <div className="print-area" style={{ color: "#1a1208", fontFamily: "'Inter', Arial, sans-serif", fontSize: 11.5, padding: "0 4px" }}>
      ...
      <div style={{ textAlign: "center", borderBottom: "2px solid #800020", paddingBottom: 6, marginBottom: 10 }}>
        <img src={logo} alt="Mount Carmel College" style={{ height: 44, margin: "0 auto 3px", display: "block" }} />
      ...
      <div style={{ display: "flex", justifyContent: "space-between", gap: 24, marginTop: 10, alignItems: "flex-start", breakInside: "avoid" }}>
        ...
        <div style={{ width: "42%", minWidth: 220 }}>
          <div style={{ display: "flex", justifyContent: "space-between", padding: "5px 10px", borderBottom: "1px solid #c9bda6" }}>
            <span>Total</span><b>{rupee(c.totalAmount)}</b>
          </div>
          <div style={{ display: "flex", justifyContent: "space-between", padding: "5px 10px", borderBottom: "1px solid #c9bda6" }}>
