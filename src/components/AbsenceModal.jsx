...
/* Per-subject absence sessions popup (shared by the Attendance and
...
export default function AbsenceModal({ subject, onClose }) {
  const [rows, setRows] = useState(null);
  useEffect(() => {
    unwrap(api.get(`/attendance/absence-periods/${subject.subjectId}`, { skipErrorToast: true }))
      ...
      >
        ...
        <div className="max-h-[62vh] overflow-y-auto p-0">
          {rows === null ? (
            ...
            <p className="p-8 text-center text-sm text-muted-foreground">No absence sessions found.</p>
          ) : (
